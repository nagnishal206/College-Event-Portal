...
def create_app() -> Flask:
    ...
    # Session cookie config: for local HTTP development, use Lax and Secure=False.
    # For Replit cross-origin iframe preview, these must be "None" and True.
    is_local = os.environ.get("FLASK_DEBUG", "1") == "1"
    app.config["SESSION_COOKIE_SAMESITE"] = "Lax" if is_local else "None"
    app.config["SESSION_COOKIE_SECURE"] = not is_local
    app.config["SESSION_COOKIE_HTTPONLY"] = True
    app.config["REMEMBER_COOKIE_SAMESITE"] = "Lax" if is_local else "None"
    app.config["REMEMBER_COOKIE_SECURE"] = not is_local
